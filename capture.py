...
Captures 4K images on motion detection and records complete session video
"""
...
class MotionCapture4K:
    def __init__(self):
        # Fixed configuration - no user input
        self.camera_index = 0
        self.target_width, self.target_height = 3840, 2160  # Try 4K first
        self.width, self.height = 3840, 2160  # Will be updated after camera init
        self.resolution_name = "4K"  # Will be updated if fallback occurs
        self.jpeg_quality = 90
        self.motion_threshold = 50000  # 50k pixel changes
    ...
    def initialize_camera(self):
        """Initialize camera with 4K first, fallback to 1080p if needed"""
        try:
            ...
            # Check if 4K was achieved
            if actual_width == 3840 and actual_height == 2160:
                # 4K successful
                self.width, self.height = 3840, 2160
                self.resolution_name = "4K"
                self.jpeg_quality = 90
                self.logger.info("✅ 4K Camera initialized successfully:")
            else:
                # 4K failed, fallback to 1080p
                self.logger.warning("❌ 4K resolution not achieved, falling back to 1080p...")
                ...
                # Verify 1080p settings
                actual_width = int(self.cap.get(cv2.CAP_PROP_FRAME_WIDTH))
                actual_height = int(self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
                actual_fps = self.cap.get(cv2.CAP_PROP_FPS)
                
                # Update instance variables for 1080p
                self.width, self.height = 1920, 1080
                self.resolution_name = "1080p"
                self.jpeg_quality = 95  # Higher quality for smaller files
    ...
    def initialize_video_writer(self):
        ...
        try:
            ...
            # Create video writer with 4K resolution at 25fps
    ...
    def detect_motion(self, frame):
        """Detect motion using optimized 4K processing"""
        if self.prev_frame is None:
            self.prev_frame = frame.copy()
            return False
        
        # Use downscaled frames for motion detection (performance optimization)
        small_frame = cv2.resize(frame, (960, 540))  # 1/4 size
        small_prev = cv2.resize(self.prev_frame, (960, 540))
        ...
        # Scale motion threshold for smaller comparison (1/16 of original)
        effective_threshold = self.motion_threshold // 16
        
        # Update previous frame
        self.prev_frame = frame.copy()
        
        return motion_pixels > effective_threshold
    ...
    def stop_capture(self):
        ...
        if self.frame_count > 0:
            ...
            if self.saved_images > 0:
                ...
                avg_image_size = 0.65 if self.resolution_name == "4K" else 0.25  # MB per image
...
def main():
    ...
    print("🎥 4K Motion Capture with Video Recording")
    print("=========================================")
    print("Configuration:")
    print("  • Resolution: 4K (3840x2160)")
    print("  • Mode: Motion detection")
    print("  • Threshold: 50,000 pixel changes")
